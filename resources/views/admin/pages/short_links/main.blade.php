...
@section('titleSuffix')| <a href="{!! route('admin.short_links.add') !!}" class="text-cyan"><i
                ...
                <table class="table table-striped m-b-0 columns-middle">
                    ...
                    @foreach($items as $item)
                        <tr class="item-row" data-id="{!! $item->id !!}">
                            ...
                            @endif
                            <td>
                                <a href="{{ route('admin.short_links.edit', ['id'=>$item->id]) }}"
    ...
    <script>
        ...
        $('#itemDeleteForm').on('submit', function () {
            ...
            if (thisItemId && thisItemId.match(/^[1-9][0-9]{0,9}$/)) {
                ...
                $.ajax({
                    url: '{!! route('admin.short_links.delete') !!}',
                    type: 'post',
                    dataType: 'json',
                    data: {
                        _token: csrf,
                    ...
                    success: function (e) {
                        if (e.success) {
                            loader.removeClass('shown');
                            blocked = false;
                            toastr.success('Короткая ссылка удалена');
                            modal.modal('hide');
