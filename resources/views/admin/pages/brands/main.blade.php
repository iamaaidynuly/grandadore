...
@section('titleSuffix')| <a href="{!! route('admin.brands.add') !!}" class="text-cyan"><i class="mdi mdi-plus-box"></i>
...
@section('content')
                ...
                <table class="table table-striped m-b-0 columns-middle">
                    ...
                    @foreach($items as $item)
                        <tr class="item-row" data-id="{!! $item->id !!}">
                            ...
                            @endif
                            <td>
                                <a href="{{ route('admin.brands.edit', ['id'=>$item->id]) }}"
                                   {!! tooltip('Редактировать') !!} class="icon-btn edit"></a>
                                <a href="{{ route('admin.gallery', ['gallery'=>'brands_item', 'key'=>$item->id]) }}"
                                   {!! tooltip('Галерея') !!} class="icon-btn gallery"></a>
                                {{--<a href="{{ route('admin.video_gallery', ['gallery'=>'brands_item', 'id'=> $item->id]) }}"
    ...
    <p class="font-14">Вы действительно хотите удалить данную новость?</p>
    @endmodal
@endsection
@push('css')
@endpush
