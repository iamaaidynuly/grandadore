...
@section('content')
                ...
                <table class="table table-striped m-b-0 columns-middle">
                    ...
                    @foreach($items as $item)
                        <tr class="item-row" data-id="{!! $item->id !!}">
                            <td class="item-title">{{   _cutStringByWords($item->message,50)  }}</td>
                            ...
                            @endif
                            <td>
                                <a href="{{ route('admin.reviews.moderate', ['id'=>$item->id]) }}"
                                   {!! tooltip('Разрешить') !!} class="icon-btn">
                                    <i class="fa fa-ban" style=" {{ ($item->moderated)?'color: green':'color: red'}}"
                                       aria-hidden="true"></i>

                                </a>
                                <a href="{{ route('admin.reviews.view', ['id'=>$item->id]) }}"
    ...
    <p class="font-14">Вы действительно хотите удалить данный отзыв?</p>
    @endmodal
@endsection
@push('css')
@endpush
