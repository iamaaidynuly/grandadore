...
@section('titleSuffix')| <a href="{!! route('admin.addSearch') !!}" class="text-cyan"><i
            ...
            <table class="table table-striped m-b-0 columns-middle init-dataTable">
                ...
                @foreach($search as $row=>$value)
                    <tr class="item-row thisRow{{$value->id}}" data-id="21212" data-toggle="collapse" href="#collapse{{$row}}" role="button"
                        ...
                        <td onclick="location.href='{{ route('admin.editSearch',['id'=>$value->id]) }}'">{{ $row+1 }}</td>
                        <td onclick="location.href='{{ route('admin.editSearch',['id'=>$value->id]) }}'">{{ $value->title }}</td>
                        ...
                        <td>
                        <form action="{{ route('admin.searchRemove') }}" method="post">
                            @csrf
    ...
    <script>
       ...
       function change(id) {
            $.ajax({
                ...
                url: '{{ route('admin.searchChangeStatus') }}',
                data: {
                    "_token": "{{ csrf_token() }}",
                    'id' : id ,
                },
