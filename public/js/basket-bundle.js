...
var BasketBundleClass = function BasketBundleClass() {
        ...
        while (1) {
          switch (_context2.prev = _context2.next) {
            case 0:
              ...
              colorId = _args2.length > 3 && _args2[3] !== undefined ? _args2[3] : null;
              dataToPush = {
                itemId: itemId,
                count: count
              };
  ...
  self.setBasketCount = function () {
    ...
    if (self.isProductView) {
      self.basketItems.forEach(function (item) {
        $("".concat(basketCalculator.itemCountInputSelector, ", .product-view-count")).filter("[data-item-id=\"".concat(item.itemId, "\"]")).val(item.count);
      });
    }
  };
  ...
  self.animateAdding = function (element) {
    ...
    var imgToDrag = element.find("img.animatable-image").eq(0);
  ...
  $(document).on('click', self.basketActionElementSelector, function () {
    ...
    if (itemId && self.getItem(itemId) === null) {
      ...
      if (sizesCount >= 0) {
        ...
        var sizeElements = $('.filter__content .filter-item');
      ...
      var colorId = $('.color-data-id').data('id');

        if(colorId != undefined && size != undefined) {

            self.addItem(itemId, count, size, colorId).then(function () {
                self.initTriggersActives();

                if (!self.isProductView) {
                    self.animateAdding($(_this).closest('.product-card'));
                }

                self.constructSmallBasket();
                $('.choose-color-red').css({'border':'1px solid #CECECE'})
                $('.choose-size-red').css({'border':'1px solid #CECECE'})
            ...
            self.setBasketCount();
        }else if(colorId == undefined && size == undefined){

        $('.choose-color-red').css({'border':'1px solid red'})
         $('.choose-size-red').css({'border':'1px solid red'});
    ...
    self.updateItemRequest(itemId, value).then(function () {
      self.removeItem(itemId);
    ...
    if (itemId && self.getItem(itemId)) {
      self.updateItemRequest(itemId, value).then(function (r) {
        self.removeItem(itemId);
        self.basketItems.push({
          itemId: itemId,
          count: value
...
module.exports = __webpack_require__(/*! C:\OpenServer\domains\grandadore\resources\js\basket-bundle.js */"./resources/js/basket-bundle.js");
