...
var BasketBundleClass = function BasketBundleClass() {
  ...
  $(document).on('click', self.basketActionElementSelector, function () {
    ...
    if (itemId && self.getItem(itemId) === null) {
      ...
      self.addItem(itemId, count, size).then(function () {
        self.initTriggersActives();
    ...
    if (itemId) {
      ...
      $('.small-basket-wrapper .basket__body .basket-item-row').each(function () {
        if (parseInt($(this).attr('data-item-id')) == itemId) {
          _count = $(this).find('.basket-item-count').val();
        }

        countAll += parseInt($(this).find('.basket-item-count').val());
      });
      _count++;
      self.updateItemRequest(itemId, _count).then(function () {
        self.initTriggersActives();
        self.constructSmallBasket();
...
module.exports = __webpack_require__(/*! /home/poghos/Projects/grandadore/resources/js/basket-bundle.js */"./resources/js/basket-bundle.js");
